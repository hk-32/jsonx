...
/*func printable(Byte scanner.Token) interface{} {
	switch word := Byte.(type) {
...
// BuildOne -
func BuildOne(input *bufio.Reader) (interface{}, error) {
	// Receive a token
	token := scanner.LexOne(input)

	// Handle token depending on its type
	switch word := token.(type) {
	case error:
		return nil, ErrDefault
	...
	case scanner.Literal:
		// Parse Arrays
		if word == scanner.LeftBracket {
			var holder Array
			for {
				if token, err := BuildOne(input); err == nil {
					holder = append(holder, token)
					// Swallow next ... should be a comma or a RightBracket
					next := scanner.LexOne(input)
					if next == scanner.Comma {
						continue
					} else if next == scanner.RightBracket {
						break
					}
				} else if token == scanner.RightBracket {
					// Array had nothing...
					return holder, nil
				}
				// Error... next can be colon or something that does not make sense here
		...
		// Parse Objects
		if word == scanner.LeftBrace {
			var holder = make(Object)
			for {
				token = scanner.LexOne(input)
				if key, isString := token.(string); isString {
					// Swallow a colon
					if scanner.Colon == scanner.LexOne(input) {
						// Parse value
						if value, err := BuildOne(input); err == nil {
							holder[key] = value
							// Swallow next ... should be a comma or a RightBrace
							next := scanner.LexOne(input)
							if next == scanner.Comma {
								continue
							} else if next == scanner.RightBrace {
								break
							}
						}
					}
				} else if token == scanner.RightBrace && len(holder) == 0 {
					// Object had nothing...
					return holder, nil
				}
				// Error... next can be colon or something that does not make sense here
		...
		// Literal must be some other type, and is probably placed as a value or key
		return word, ErrDefault
	}

	// Should be a string, number, bool or null/nil
	return token, nil
}

// Build -
func Build(source interface{}) (interface{}, error) {
	var input *bufio.Reader

	switch v := source.(type) {
	case []byte:
		input = bufio.NewReader(bytes.NewReader(v))
	case string:
		input = bufio.NewReader(strings.NewReader(v))
	case *os.File:
		input = bufio.NewReader(v)
	case *bufio.Reader:
		input = v
	default:
		panic("Unsuported source input")
	}
	return BuildOne(input)
}*/
